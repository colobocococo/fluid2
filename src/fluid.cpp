...
Fixed <30, 13> p[N][M]{}, old_p[N][M];

struct VectorField {
    array<Fixed <30, 13>, deltas.size()> v[N][M];
...
tuple<Fixed <30, 13>, bool, pair<int, int>> propagate_flow(int x, int y, Fixed <30, 13> lim) {
    last_use[x][y] = UT - 1;
    Fixed <30, 13> ret = 0;
...
struct ParticleParams {
    char type;
    Fixed <30, 13> cur_p;
    array<Fixed <30, 13>, deltas.size()> v;
